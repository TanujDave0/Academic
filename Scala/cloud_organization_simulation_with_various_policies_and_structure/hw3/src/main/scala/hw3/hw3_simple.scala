...
import HelperUtils.{CreateLogger, Parameters1}
...
object hw3_simple:
  ...
  import HelperUtils.Parameters1.*
  private val logger = CreateLogger(classOf[hw3_simple.type]) // creating a logger for it

  // getting all the variables from the conf for this simulation

  // for hosts
  val HOSTS = Parameters1.HOSTS
  val HOST_PES = Parameters1.HOST_PES
  val HOST_MIPS = Parameters1.HOST_MIPS
  val HOST_RAM = Parameters1.HOST_RAM
  val HOST_BW = Parameters1.HOST_BW
  val HOST_STORAGE = Parameters1.HOST_STORAGE

  // for VMs
  val VMS = Parameters1.VMS
  val VM_PES = Parameters1.VM_PES
  val VM_MIPS = Parameters1.VM_MIPS
  val VM_RAM = Parameters1.VM_RAM
  val VM_BW = Parameters1.VM_BW
  val VM_SIZE = Parameters1.VM_SIZE
  val VM_SCALING_FACTOR = Parameters1.VM_SCALING_FACTOR
  val VM_SCALING_UPPER_THRESHOLD = Parameters1.VM_SCALING_UPPER_THRESHOLD
  val VM_SCALING_LOWER_THRESHOLD = Parameters1.VM_SCALING_LOWER_THRESHOLD

  // for Cloudlets
  val CLOUDLETS = Parameters1.CLOUDLETS
  val CLOUDLET_PES = Parameters1.CLOUDLET_PES
  val CLOUDLET_LENGTH = Parameters1.CLOUDLET_LENGTH
  val CLOUDLET_SIZE = Parameters1.CLOUDLET_SIZE
  val CLOUDLET_DYNAMIC_USAGE = Parameters1.CLOUDLET_DYNAMIC_USAGE

  val INTERVAL = Parameters1.INTERVAL
  ...
  // the datacentre
  // uses first available host as the assignment policy
  val datacentre0 = createDatacenterFirstHost()

  // the broker
  val broker0 = new DatacenterBrokerSimple(simulation)

  // the list of cloudlets to be submitted
...
//      temp2.setCloudletScheduler(new CloudletSchedulerSpaceShared())
  ...
  @main def hw3_simple_main() = {simulate_simple()}
