...
* */
object Parameters1:
  private val logger = CreateLogger(classOf[Parameters1.type])
  val config = ObtainConfigReference("homework3_1") match {
...
//    case Long => Long
  ...
  private def func4Parameter[T](defaultVal: T, f: String => T): String => T =
    (pName: String) => Try(f(s"homework3_1.$pName")) match {
...
//      case v: Long => func4Parameter(v, config.getLong)(pName)
  ...
  val INTERVAL = getParam("INTERVAL", 2)