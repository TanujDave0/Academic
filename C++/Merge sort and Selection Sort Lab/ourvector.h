...
private:
  ...
  //
  static long long InUse;     // # of vector objects currently in use
  static long long Vectors;   // # of vector objects created
  static long long Inserts;   // # of elements inserted into the vector
  static long long Accesses;  // # of times elements were accessed (read or written)
...
public:
  ...
  virtual ~ourvector()
  {
    ...
    {
      //_stats(); --- must be called explicitly
    }
  }
  ...
  ourvector& operator=(ourvector&& other)
  {
    ...
    //
    //  since we have 2 vectors and are now moving one into another,
    //  one is effectively being destroyed:
    InUse--;
...
//
template<typename T>
long long ourvector<T>::InUse = 0;

template<typename T>
long long ourvector<T>::Vectors = 0;

template<typename T>
long long ourvector<T>::Inserts = 0;

template<typename T>
long long ourvector<T>::Accesses = 0;
