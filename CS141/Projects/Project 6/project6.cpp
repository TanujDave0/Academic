...
class Node{
    public:
        vector<char> storeBoardPieces = vector<char>(36);
        char storePlayerToMove;
        int moveNumber;
        Node* previousNode;
};

void addNodeToList(Node* &pHead, int numberOfIteration, vector<char> boardPieces, char playerToMove){
    // Create a new node and store current values into it
    Node* temprory = new Node;
    temprory->moveNumber = numberOfIteration;
    temprory->previousNode = pHead;
    temprory->storeBoardPieces = boardPieces;
    temprory->storePlayerToMove = playerToMove;
    pHead = temprory; // Prepend it onto the front of the list
}

void displayList(Node *pHead){
    if(pHead->previousNode==nullptr){
        cout<<pHead->moveNumber<<":"<<pHead->storePlayerToMove<<endl;
    }
    else{
        cout<<pHead->moveNumber<<":"<<pHead->storePlayerToMove<<"->";
        displayList(pHead->previousNode);
    }
} // end displayList()
...
class Pentago{
    public:
        ...
        void makeMove(char row, char columnInt, char quadrantInt, char direction, char playerToMove, int& win); // function for maing move.
        ...
        vector<char> returnBoardPieces() {return boardPieces;};
        void boardPiecesChanger(Node* pHead);
...
void Pentago::boardPiecesChanger(Node* pHead){
    boardPieces = pHead->storeBoardPieces;
}
...
void Pentago::displayBoard(){
    cout<<"    1   2   3   4   5   6\n"
        ...
        <<"  3-----------------------4\n"
        <<"    1   2   3   4   5   6 \n"
        <<"   List of moveNumber:playerToMove is ";
}
...
void Pentago::makeMove(char rowInt, char columnInt, char quadrantInt, char direction, char playerToMove, int& win){
    int playersWon=0;
    char playerWonChar;

    boardPieces.at((rowInt*6)+columnInt-1)=playerToMove; // making the move.
...
void deleteNodeFromList(Node* &pHead, Pentago& theBoard, char &playerToMove){
    Node* temprory = pHead;
    pHead = pHead->previousNode;
    theBoard.boardPiecesChanger(pHead);
    playerToMove = pHead->storePlayerToMove;
    delete temprory;
} //end deleteNodeFromList()

// function that takes input.
void takeInput(int &numberOfIteration, char& row, char& column, char& quadrant, char& direction, char &playerToMove, Pentago& theBoard, Node* &pHead) {
    cout<<numberOfIteration<<". Enter row, column, quadrant, direction for "<<playerToMove<<": ";
    cin>>row;
    ...
    else if(toupper(row)=='U'){ // checking if player wants undo.
        if(pHead->moveNumber!=1){
            cout<<"* Undoing move *"<<endl;
            numberOfIteration--;
            deleteNodeFromList(pHead, theBoard, playerToMove);
        }
        else{
...
void changeplayerToMove(char& playerToMove, int& numberOfIteration){
    if(playerToMove == 'X')
        playerToMove = 'O';
    else
        playerToMove = 'X';
...
int main(){
    ...
    Node* pHead = nullptr;
    addNodeToList(pHead, numberOfIteration, theBoard.returnBoardPieces(), playerToMove);
    ...
    while(numberOfIteration<=36){
        ...
        displayList(pHead);
        takeInput(numberOfIteration, row, column, quadrant, direction, playerToMove, theBoard, pHead); // taking input.
        ...
        if(toupper(row)!='I' && toupper(row)!='U' && theBoard.checkInput(rowInt, columnInt, quadrantInt, direction)){ // if inputs are right. 
            theBoard.makeMove(rowInt, columnInt, quadrantInt, direction, playerToMove, win); // making the move.
            if(win>0) // if win conditons met.
                break; // exiting out of the main loop.
            changeplayerToMove(playerToMove, numberOfIteration); // changing the player symbol.
            addNodeToList(pHead, numberOfIteration, theBoard.returnBoardPieces(), playerToMove);
        }
    }
