// Prog6PentagoUndo.cpp
//     Play the two-player game of Pentago, trying to get 5 pieces in a row.
//
// Author: Dale Reed
// Program: #6, Pentago using linked list for infinite undo of moves
//
 /* Running the program looks like what is shown below.
		...
		F | .   .   . | .   .   . | F
		  3-----------------------4
		    1   2   3   4   5   6
		   List of moveNumber:playerToMove is 1:X
		1. Enter row, column, quadrant, direction for X: u
		*** You cannot undo past the beginning of the game.  Please retry. ***
		...
		F | .   .   . | .   .   . | F
		  3-----------------------4
		    1   2   3   4   5   6
		   List of moveNumber:playerToMove is 1:X
		1. Enter row, column, quadrant, direction for X: a1 1R
		...
		F | .   .   . | .   .   . | F
		  3-----------------------4
		    1   2   3   4   5   6
		   List of moveNumber:playerToMove is 2:O->1:X
		2. Enter row, column, quadrant, direction for O: u
		* Undoing move *
		...
		F | .   .   . | .   .   . | F
		  3-----------------------4
		    1   2   3   4   5   6
		   List of moveNumber:playerToMove is 1:X
		1. Enter row, column, quadrant, direction for X: x
		Exiting program...
...
class Board
{
    public:
        ...
        Board( const Board & oldBoard) {
            // Do a member by member copy of elements from old board into new board
            for( int i=0; i<BoardSize; i++) {
                theBoard[ i] = oldBoard.theBoard[ i];
            }
        }
...
//-------------------------------------------------------------------------------------
// If move is invalid, return 1 for true, else return 0 for false.
        ...
        char rotateDirection)     // Direction ('L' or 'R') to rotate the chosen quadrant
{
    ...
    if( moveRow < 'A' || moveRow > 'F') {
        ...
        return 1;  // 1 is true, since move IS invalid
    }
    ...
    return 0;  // 0 is false, since move is NOT invalid
}//end moveIsInvalid()
...
{
    while( pTemp != NULL) {
        cout << pTemp->oldMoveNumber << ":" << pTemp->oldPieceToMove;
        if( pTemp->pNext != NULL) {
            cout << "->";
        }
        pTemp = pTemp->pNext;   // Advance to next list node
    }
    cout << endl;
} // end displayList()
...
void deleteNodeFromList(
        ...
        Board &theBoard,        // Current game board to be restored from list
        char &pieceToMove,      // The piece to move to be restored from list
        int &moveNumber)        // Current move number to be restored from list
{
    ...
    pieceToMove = pHead->oldPieceToMove;
...
void addNodeToList(
        ...
        char pieceToMove,         // The piece to move, to be added to the list node
        int theMoveNumber)        // The moveNumber, to be added to the list node
{
    ...
    pTemp->oldPieceToMove = pieceToMove;
    pTemp->oldMoveNumber = theMoveNumber;
    pTemp->pNext = pHead;
...
int main() {
    ...
    Node *pHead = NULL;     // Linked list head pointer, used for undoing moves.
    ...
    addNodeToList( pHead, theBoard, pieceToMove, moveNumber);
    ...
    while( theBoard.notDone() ) {
        ...
        while( true) {
            ...
            if( moveRow == 'U') {
                ...
                deleteNodeFromList( pHead, theBoard, pieceToMove, moveNumber);
                continue;   // Go back up to top of loop to display board and prompt for next move
            }

            // Now that we made sure the first character of input was not 'x' to exit,
            //    'i' for instructions, or 'u' to undo a move, now read in column for the
            //    move, quadrant to be rotated, and rotate direction
